#!/usr/bin/env bash
# ============================================================================
# IBM - Netzwerksuche Deye Hybrid (RS485-Gateway)
#
# Findet das RS485-Ethernet-Gateway des Wechselrichters im lokalen Netz
# wieder, wenn der Router ihm per DHCP eine neue IP zugeteilt hat, und
# traegt die neue Adresse ueber die openHAB REST API in die
# Modbus-TCP-Bridge ein. Erkannt wird die Anlage an einer Modbus-FC03-Probe
# auf das Work-Mode-Register (142) an Slave 1 mit plausiblem Wert (0-2).
#
# Eine per Netz lesbare Seriennummer gibt es nicht - eine neue Adresse wird
# deshalb nur uebernommen, wenn GENAU EIN passendes Gateway im Netz
# antwortet. Stehen mehrere solche Gateways im selben Netz, muss die IP von
# Hand im Bridge-Thing gepflegt werden (das Skript bricht dann mit einem
# Hinweis ab). Am besten dem Gateway im Router eine feste IP-Reservierung
# geben. Achtung: gesucht wird fest auf Port 502 - laeuft das Gateway auf
# einem anderen Port, findet die Suche nichts.
#
# Vorlage im Repository: inverters/deye/rediscover.sh - die Platzhalter
...
# Slave-Adresse des Wechselrichters am RS485-Bus (Deye-Vorgabe 1)
MODBUS_UNIT_ID=1
...
# Antwortet unter der IP ein Deye-Gateway (Modbus-FC03 auf 142, Slave 1,
# Wert plausibel 0-2)?
probe() {
...
try:
    ...
    resp = s.recv(256)
    s.close()
    ok = (len(resp) >= 11 and resp[6] == unit and resp[7] == 3
          and struct.unpack(">H", resp[9:11])[0] <= 2)
...
# dann bringt eine Netzwerksuche nichts (z. B. RS485-Verkabelung oder
# Wechselrichter aus).
if [ -n "$current_host" ] && probe "$current_host"; then
  log "Gateway antwortet weiterhin unter $current_host - keine Suche."
  log "Ursache liegt nicht an der Adresse (RS485-Verkabelung? Wechselrichter aus? openhab.log pruefen)."
  exit 0
fi
...
if [ -z "$candidates" ]; then
  log "Kein Deye-Gateway im Netz gefunden - Gateway aus, anderer Port oder Wechselrichter ohne Antwort?"
