...
detect_grid_power_items() {
  ...
  if [ -z "$found" ] && [ -n "$INVERTER_GRID_POWER_CHANNEL" ] && [ -f "$OPENHAB_CONF/items/ibm.items" ]; then
    found="$(grep -E "channel=\"[^\"]*:${INVERTER_GRID_POWER_CHANNEL}\"" "$OPENHAB_CONF/items/ibm.items" 2>/dev/null \
             | awk '{print $2}' | sort -u || true)"
  fi
