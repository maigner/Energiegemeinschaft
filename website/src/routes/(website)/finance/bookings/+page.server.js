import { getLabels, getBookings, getBookingsLabels, getBookingsAttachments, getAttachment, addFileToBooking, deleteFileFromBooking } from '$lib/server/db/finance/bookings';
...
// Actions are reachable by every logged-in member (the hook only checks for a
// session on /finance), so each one has to enforce the cashier role itself.
const requireCashier = async (locals) => {
...
export const actions = {
    downloadFile: async ({ request, locals }) => {
        await requireCashier(locals);
