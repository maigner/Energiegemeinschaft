<script>
    ...
    const now = new Date();
    const nowLabel = formatDate(now) + " " + formatTime(now);

    // find the closest category to now
    const categories = forecast.map(
        (e) => formatDate(e.time) + " " + formatTime(e.time),
    );
    const closest = categories.reduce((prev, curr) => {
        const prevTime = new Date(prev).getTime();
        const currTime = new Date(curr).getTime();
        return Math.abs(currTime - now) < Math.abs(prevTime - now)
            ? curr
            : prev;
    });
