...
Eigenbedarf ist die gelernte Hauslast $P_{\text{Haus}}$
(Abschnitt~\ref{sec:hauslast}, Rückfall 300\,W) über die Reservedauer
$T_{\text{Reserve}}$ mit einem Sicherheitszuschlag von 30\%:
\begin{equation}
  E_{\text{Reserve}} \;=\; \frac{P_{\text{Haus}}}{1000} \cdot T_{\text{Reserve}}
    \cdot 1{,}3.
\end{equation}
$T_{\text{Reserve}}$ reicht vom aktuellen Zeitpunkt bis zum
Vormittags-Crossover. Liegt die Wolkenvorschau für das nächste
Sonnenfenster über der Wolkenschwelle (oder fehlt sie), hängt das Haus
auch tagsüber an der Batterie: Dann verlängert sich die Reservedauer bis
zum Abend-Crossover des Folgetags, und bei kleinen Batterien oder hoher
Hauslast bleibt so kein Budget übrig; die Batterie behält ihren Inhalt
für das eigene Haus. Wie das Gateway daraus den Ziel-Ladestand bildet,
zeigt Gleichung~\eqref{eq:ziel}.
...
\begin{itemize}
  \item \textbf{Trübe Vorschau:} Liegt die Vorschau über der
    Wolkenschwelle (dieselbe wie bei der Ladesperre), wird nicht mehr hart
    gestoppt, sondern die Reservedauer des Nachtbudgets bis zum nächsten
    Abend verlängert (Abschnitt~\ref{sec:nachtbudget}): Die Batterie
    speist nur ein, was das Haus auch an einem bedeckten Folgetag nicht
