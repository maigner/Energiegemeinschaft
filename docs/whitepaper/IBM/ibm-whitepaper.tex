...
genügen dafür: Am sonnigen Vormittag wird das Laden der Batterie verzögert,
damit PV-Überschuss in die Gemeinschaft fließt, solange diese ihn braucht,
und die Batterie mitten in der Überschussspitze statt direkt nach
Sonnenaufgang lädt. In der Nacht speist die Batterie kontrolliert in das
Netz ein, solange die Gemeinschaft mehr verbraucht als erzeugt. Beide
...
\begin{itemize}
  \item \textbf{Vormittag (Ladesperre):} Die Batterie beginnt so spät wie
    möglich zu laden, wird aber bis zum Abend trotzdem voll. Der Überschuss
    des frühen Vormittags deckt die morgendliche Verbrauchsspitze der
    Gemeinschaft direkt.
...
Kern im selben Regel-Body voranstellt. Der Adapter-Kontrakt umfasst genau
drei Funktionen:
...
\code{ibm\_status\_push} & Zustand an das Dashboard melden & alle 5\,min \\
...
\code{/api/ibm/ladefenster/v1} & POST & individuelles Fenster + Nachtbudget & Token \\
\code{/api/eeginfo/crossover/v1} & GET & Crossover-Zeiten der Woche & öffentlich \\
\code{/api/wolken/vorschau/v1} & GET & Bewölkung im nächsten Mittagsfenster & öffentlich \\
...
Gleichung~\eqref{eq:localend} aus.
...
Open-Meteo-Vorhersagedaten. Fehlen Wetterdaten, antwortet die API bewusst
mit einem Fehler statt mit 0, denn 0\% würde von den Gateways als voller
Sonnenschein gelesen.

% ============================================================================
\section{Teil A: Ladesperre am Vormittag}\label{sec:ladesperre}
% ============================================================================

\subsection{Grundprinzip}

Am sonnigen Vormittag wird das Laden der Batterie in Fünf-Minuten-Schritten
gesperrt. Das Sperrfenster kommt aus der Tagesprognose des Servers: vom
ersten Sonnenschein bis in die Mittagsspitze des Gemeinschafts-Überschusses.
Das Fenster gilt nur für das mitgelieferte Datum; ein nach einem API-Ausfall
übrig gebliebenes Fenster von gestern sperrt heute nicht. Zusätzlich wird
nur bei sonniger Vorschau gesperrt: Liegt die Bewölkungsvorschau über der
Schwelle (Vorgabe 75\%), bleibt das Laden erlaubt, denn an einem trüben Tag
würde die Batterie sonst womöglich gar nicht mehr voll.

\subsection{Individualisiertes und lokal berechnetes Sperr-Ende}

Das Community-Fenster des Servers kennt weder Batteriegröße noch
Ladeleistung der einzelnen Anlage. Für das Sperr-Ende gibt es deshalb drei
Stufen, in absteigender Priorität:
...
\begin{enumerate}
  \item \textbf{Individualisiertes Server-Ende:} Anlagen mit Status-Token
    ...
    die Batterie am Abend gerade noch voll wird. Die Antwort ist als
    individuell markiert; der Kern übernimmt sie unverändert.
...
Der Fensterbeginn (erster Sonnenschein) und die Datumsprüfung kommen immer
vom Server. Plausibilitätsfenster verwerfen Unsinniges: Der Beginn muss
zwischen 4 und 12~Uhr liegen, das Ende zwischen 5 und 15~Uhr.

\subsection{Wirkung für die Gemeinschaft}

Die morgendliche Verbrauchsspitze der Gemeinschaft wird direkt aus der PV
gedeckt, und die Batterien laden mitten in der Überschussspitze statt
unmittelbar nach dem Crossover. Das hält die Gemeinschaft nach dem
Crossover im Plus, fängt Abregelungsverluste und verkürzt nebenbei die
Standzeit der Batterie bei 100\% Ladung.
...
95\% liegt (darüber drosselt der Wechselrichter selbst). Steigt der
Ladestand um mindestens 5~Prozentpunkte, ergibt geladene Energie (aus der
Kapazität) geteilt durch die Messdauer eine Stichprobe in kW.
...
\begin{itemize}
  ...
  \item \textbf{Kein Eingriff ohne Daten:} Ohne plausible Crossover-Zeiten
    keine Entladung; ohne gültiges, datumsrichtiges Ladefenster keine
    Sperre; ohne frische Wolkenvorschau (älter als drei Stunden) keine
    Sperre und nur minimale Entladung.
...
Auf Wunsch meldet jede Anlage alle fünf Minuten ihren Zustand an den
Server: Ladestand, Wechselrichter-Status, Batterie-, Netz-, PV- und
Lastleistung, alle Schalterstellungen, die gelernten Kennwerte, die zuletzt
empfangenen Steuersignale, die Warnungen und Fehler aus dem openHAB-Log der
letzten 24~Stunden, Versionsstände (IBM-Paket, openHAB, Java,
Betriebssystem), ausstehende Betriebssystem-Updates und den Systemzustand
des Pi (CPU-Temperatur, Unterspannungs-Register, SD-Karten-Füllstand,
