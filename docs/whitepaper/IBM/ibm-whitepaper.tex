...
nächsten 24~Stunden), zieht eine Eigenbedarfsreserve ab und behält einen
Abschlag von 20\% ein. Die Reserve ist die gelernte Hauslast
$P_{\text{Haus}}$ über die Dauer $T_{\text{Reserve}}$, in der die
Gemeinschaft in denselben 24~Stunden laut Prognose keinen Überschuss hat
(Erzeugung unter Verbrauch): nachts immer, an einem trüben Folgetag auch
tagsüber.
\begin{equation}
  B \;=\; \max\Bigl(0,\; E_{\text{ladbar,24h}}
    - \frac{P_{\text{Haus}}}{1000} \cdot T_{\text{Reserve}}\Bigr) \cdot 0{,}8
    \cdot f_{w}.
\end{equation}
Der Wolkenfaktor $f_w$ gleicht aus, dass der Prognoselauf mehrere Tage alt
sein kann, während die Wolkenvorschau für das nächste Sonnenfenster
stündlich nachgeladen wird: Bis zur Wolkenschwelle (85\%) ist $f_w = 1$,
darüber fällt er linear auf 0,4 bei 100\% Bewölkung. Bei geschlossener
Wolkendecke liefert eine PV-Anlage typisch noch ein Viertel bis die Hälfte
eines klaren Tages, und das Profil hat den trüben Tag meist schon teilweise
eingepreist; ein Faktor nahe null wäre also doppelt vorsichtig.
Das Wetter des Folgetags steckt damit genau einmal im Budget, im ladbaren
Ertrag und in der Reservedauer. Ein pauschaler Zuschlag von 24~Stunden
Hauslast wäre dagegen eine Doppelzählung und drückte bei großen Batterien
mit vorsichtig gelernter Ladeleistung selbst an klaren Tagen jedes Budget
auf null. Meldet die Anlage keine plausible Hauslast (50 bis 3000\,W),
rechnet der Server mit 300\,W, einer typischen nächtlichen Grundlast. Bei
mehrtägigem Schlechtwetter ist das Budget schlicht null: Die Batterie
behält ihren Inhalt für das eigene Haus. Wie das Gateway das Budget in
einen Ziel-Ladestand übersetzt, zeigt Gleichung~\eqref{eq:ziel}.
...
\begin{itemize}
  \item \textbf{Trübe Vorschau:} Liegt die Vorschau über der
    ...
    gestoppt, sondern das Nachtbudget gekürzt
    (Abschnitt~\ref{sec:nachtbudget}): Die Batterie speist nur den Teil
    ein, den der bedeckte Folgetag voraussichtlich wieder nachlädt. Der
    frühere Trüb-Stopp bleibt als Rückfall für Anlagen ohne wirksames
    Budget: Dort würde sonst bis zur Reserve entladen, und über eine lange
    Nacht entlädt auch minimale Leistung die Batterie fast vollständig; am
    trüben Folgetag müsste das Mitglied dann selbst Strom zukaufen.
...
sinkt der Ladestand nicht unter die Reserve, ohne gelernte Hauslast bleibt
das Budget beim Rückfallwert null. Der Wert geht per Status-Push an den
Server, der daraus die Eigenbedarfsreserve des Nachtbudgets ableitet; der
Kreis zwischen lokalem Lernen und serverseitiger Individualisierung
schließt sich hier.
